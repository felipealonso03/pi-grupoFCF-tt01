const data = {
    usuario: [
        {
            mail:'marting@gmail.com',
            pass:'marto221',
            fotoPerfil:'images/fanatico1.webp',
            dni:35089233
        },
        {
            mail:'marcosl@gmail.com',
            pass:'marquito',
            fotoPerfil:'images/fanatico2.webp',
            dni:30983333
        },{
            mail:'leon@gmail.com',
            pass:'leo323',
            fotoPerfil:'images/fanatico3.jpeg',
            dni:20343455
        },{
            mail:'leangigo@hotmail.com',
            pass:'dro34',
            fotoPerfil:'images/fanatico4.webp',
            dni:38889121
        },{
            mail:'gastonf@gmail.com',
            pass:'gasti202',
            fotoPerfil:'images/fanatico5.webp',
            dni:40571727
        }
    ],
    posteos:[
        {
            nombreImagen:'Ferrari 250 GTO (1962)',
            url:'https://i.pinimg.com/564x/ac/f8/34/acf834ec57213dcdb6c15337501ecee8.jpg',
            piePosteo:'Cuando el lujo se encuentra con la velocidad.',
            username: 'leon',
            comentarios: [
                {
                    nombre:'marting',
                    textoComent:'Obra de arte',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Increible',
                },
                {
                    nombre:'leangigo',
                    textoComent:'Potencia tana',
                },
                {
                    nombre:'gastonf',
                    textoComent:'Que hermosura',
                },

            ]
        },
        {
            nombreImagen:'Maserati 450 S (1956)',
            url:'https://i.pinimg.com/564x/7c/d8/5a/7cd85ab161560c8904e81e3ea43c9bb2.jpg',
            piePosteo:'Un vistazo al pasado con el Maserati 450 S',
            username: 'marcosl',
            comentarios: [
                {
                    nombre:'marting',
                    textoComent:'MASERATI POWER',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Que ganas de sacarlo a rodar',
                },
                {
                    nombre:'leon',
                    textoComent:'Elegancia',
                },
                {
                    nombre:'leangigo',
                    textoComent:'Amazing designe',
                },

            ]
        },
        {
            nombreImagen:'Ferrari 250 GT PF',
            url:'https://i.pinimg.com/564x/34/4d/5a/344d5ae4685a551bd5842f62234c6c74.jpg',
            piePosteo:'Sueños hechos realidad: el icónico Ferrari.',
            username: 'leangigo',
            comentarios: [
                {
                    nombre:'leon',
                    textoComent:'Italian beauty',
                },
                {
                    nombre:'gastonf',
                    textoComent:'Grandisimo',
                },
                {
                    nombre:'leangigo',
                    textoComent:'Que año es?',
                },
                {
                    nombre:'gastonf',
                    textoComent:'A la venta?',
                },

            ]
        },
        {
            nombreImagen:'Datsun 240z',
            url:'https://i.pinimg.com/564x/d2/d1/e4/d2d1e43f3e83143388896070da16b613.jpg',
            piePosteo:'Un clásico que nunca pasa de moda.',
            username: 'leangigo',
            comentarios: [
                {
                    nombre:'leangigo',
                    textoComent:'Sueño hecho realidad',
                },
                {
                    nombre:'gastonf',
                    textoComent:'Que belleza por favorr',
                },
                {
                    nombre:'leon',
                    textoComent:'Podria usarlo todo el dia',
                },
                {
                    nombre:'marting',
                    textoComent:'DEUS',
                },

            ]
        },
        {
            nombreImagen:'1960 Aston Martin DB 4 GT',
            url:'https://i.pinimg.com/564x/1e/c0/15/1ec0157f4fdb50904651a55a2432629f.jpg',
            piePosteo:'El encanto intemporal de un clásico.',
            username: 'gastonf',
            comentarios: [
                {
                    nombre:'leon',
                    textoComent:'Dios solo te pido una bendicion como est',
                },
                {
                    nombre:'marting',
                    textoComent:'De lo mejor que vi',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Arte puro',
                },
                {
                    nombre:'marting',
                    textoComent:'Bond, James Bond',
                },

            ]
        },
        {
            nombreImagen:'Golf Gti Cabriolet',
            url:'https://i.pinimg.com/564x/6c/1b/a7/6c1ba7fa64d4c63559d34ce5a912e8f9.jpg',
            piePosteo:'Deportividad al aire libre.',
            username: 'marting',
            comentarios: [
                {
                    nombre:'leon',
                    textoComent:'Potencia alemana',
                },
                {
                    nombre:'marting',
                    textoComent:'Increible el paso del tiempo',
                },
                {
                    nombre:'gastonf',
                    textoComent:'Elegancia y poder',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Cuantos caballos tiene??',
                },

            ]
        },
        {
            nombreImagen:'Shelby Cobra',
            url:'https://i.pinimg.com/564x/91/86/0d/91860d403362e9d668937997a2018784.jpg',
            piePosteo:'La herencia de la velocidad: El Shelby Cobra.',
            username: 'marcosl',
            comentarios: [
                {
                    nombre:'leon',
                    textoComent:'The Shelby empire',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Grandioso',
                },
                {
                    nombre:'gastonf',
                    textoComent:'Los detalles en cada aspecto son increibles',
                },
                {
                    nombre:'marting',
                    textoComent:'Una verdadera locura',
                },

            ]
        },
        {
            nombreImagen:'Lamborghini Countac',
            url:'https://i.pinimg.com/564x/0f/28/1e/0f281e4ef0982aea5e8652751aedece0.jpg',
            piePosteo:'Creado para romper récords y corazones.',
            username: 'gastonf',
            comentarios: [
                {
                    nombre:'marting',
                    textoComent:'Y pensar que hacian tractores',
                },
                {
                    nombre:'marcosl',
                    textoComent:'En el lambo escuchando samba',
                },
                {
                    nombre:'leon',
                    textoComent:'Que lindo bicho',
                },
                {
                    nombre:'marting',
                    textoComent:'Rum rummm',
                },

            ]
        },
        {
            nombreImagen:'1972 BMW Turbo',
            url:'https://i.pinimg.com/564x/d2/ad/79/d2ad7943b688b9ec3ee7e64b6721f53e.jpg',
            piePosteo:'1972 BMW Turbo & The Munich Olympics',
            username: 'leon',
            comentarios: [
                {
                    nombre:'marcosl',
                    textoComent:'70s deluxe',
                },
                {
                    nombre:'marting',
                    textoComent:'Que buena epoca',
                },
                {
                    nombre:'leangigo',
                    textoComent:'Un juguete de primera',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Increible',
                },

            ]
        },
        {
            nombreImagen:'Toyota Celica GT-Four 1989',
            url:'https://i.pinimg.com/564x/50/fa/88/50fa889def831c47e0f87148aa0e28e4.jpg',
            piePosteo:'El legado Toyota en el mundo de los deportivos.',
            username: 'gastonf',
            comentarios: [
                {
                    nombre:'marting',
                    textoComent:'Japoneses a la carga',
                },
                {
                    nombre:'marting',
                    textoComent:'Indestructible',
                },
                {
                    nombre:'marcosl',
                    textoComent:'Imparable',
                },
                {
                    nombre:'leangigo',
                    textoComent:'Los japoneses siempre fueron superiores',
                },

            ]
        }
    ],
    
}

module.exports = data;